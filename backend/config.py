...
class Settings:
    def __init__(self):
        ...
        # Log trạng thái key khi khởi động
        ...
        print(f"[config] CLAUDE_API_KEY: {'SET (' + self.CLAUDE_API_KEY[:10] + '...)' if self.CLAUDE_API_KEY else 'EMPTY'}")
        print(f"[config] OPENAI_API_KEY: {'SET (' + self.OPENAI_API_KEY[:7] + '...)' if self.OPENAI_API_KEY else 'EMPTY - chua co key'}")
