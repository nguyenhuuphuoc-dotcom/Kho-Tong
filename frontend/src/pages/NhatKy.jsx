...
const ACTION_LABELS = {
  create_nk:    { label: 'Tao phieu NK',  color: 'bg-green-100 text-green-700' },
  create_xk:    { label: 'Tao phieu XK',  color: 'bg-orange-100 text-orange-700' },
  delete_phieu: { label: 'Xoa phieu',     color: 'bg-red-100 text-red-700' },
  login:        { label: 'Dang nhap',     color: 'bg-blue-100 text-blue-700' },
  manual:       { label: 'Thu cong',      color: 'bg-gray-100 text-gray-600' },
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">NHAT KY HOAT DONG</h1>
          <p className="text-gray-500 mt-1 text-sm">Theo doi toan bo thao tac tren he thong</p>
          {selectedCT
            ? <span className="inline-block mt-1 px-2 py-0.5 bg-teal-100 text-teal-700 text-xs rounded-full">📌 {selectedCT.ten_ct}</span>
            : <span className="inline-block mt-1 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">🏢 Tat ca cong trinh</span>
          }
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Tim nguoi dung, chi tiet..."
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={filterAction} onChange={e => { setFilterAction(e.target.value); setPage(0) }}
            ...
            <option value="create_nk">Tao phieu NK</option>
            <option value="create_xk">Tao phieu XK</option>
            <option value="delete_phieu">Xoa phieu</option>
            <option value="login">Dang nhap</option>
          </select>
        </div>
        <span className="text-xs text-gray-400">{filtered.length} ban ghi</span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-gray-500 font-medium">Thoi gian</th>
                <th className="text-left px-4 py-3 text-gray-500 font-medium">Nguoi dung</th>
                <th className="text-left px-4 py-3 text-gray-500 font-medium">Hanh dong</th>
                <th className="text-left px-4 py-3 text-gray-500 font-medium">Chi tiet</th>
                <th className="text-left px-4 py-3 text-gray-500 font-medium">Cong trinh</th>
              </tr>
            </thead>
            <tbody>
              {loading
                ? <tr><td colSpan={6} className="py-10 text-center text-gray-400">Dang tai...</td></tr>
                : filtered.length === 0
