...
import { getPhieuList, getCongTrinh, getChiTietPhieu } from '../api'
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <p className="text-gray-500 text-sm">Danh sach phieu nhap kho</p>
            {selectedCT
        ...
        <button onClick={loadData} disabled={loading}
          className="flex items-center gap-2 px-4 py-2 bg-green-50 hover:bg-green-100 text-green-600 rounded-lg text-sm font-medium transition-colors disabled:opacity-50">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
          <Download className="w-8 h-8 text-green-500 flex-shrink-0" />
          <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex gap-3 flex-wrap items-center">
        ...
        {!selectedCT && (
          <span className="text-xs text-gray-400 bg-gray-100 px-3 py-2 rounded-lg">Chon CT o sidebar de filter</span>
        )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-gray-500 font-medium">NCC / Doi tac</th>
            ...
            <tbody>
              {loading
                ...
                : filtered.length === 0
                  ? <tr><td colSpan={7} className="py-10 text-center text-gray-400">Khong co phieu nhap kho</td></tr>
                  : filtered.map((p, i) => (
            ...
            {!loading && filtered.length > 0 && (
              <tfoot className="bg-green-50 border-t-2 border-gray-200">
                <tr>
                  ...
                  <td className="px-4 py-3 text-right font-bold text-green-700">{formatVND(tongTien)}</td>
                  <td></td>
                </tr>
              </tfoot>
            )}
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between p-5 border-b">
              <div>
                <h3 className="font-bold text-gray-800 text-lg">
                  <span className="text-green-600">{selectedPhieu.so_phieu}</span>
                </h3>
                <p className="text-sm text-gray-500 mt-0.5">
                  {selectedPhieu.ngay} &nbsp;·&nbsp; {ctMap[selectedPhieu.cong_trinh_id] || ''}
                  {selectedPhieu.doi_tac && <> &nbsp;·&nbsp; NCC: {selectedPhieu.doi_tac}</>}
                </p>
              </div>
            ...
            <div className="p-4 border-t bg-gray-50 flex justify-between items-center text-sm">
              ...
              <span className="font-bold text-green-700 text-base">Tong: {formatVND(selectedPhieu.tong_tien)}</span>
            </div>
          </div>
        </div>
      )}
